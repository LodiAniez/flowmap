...
export function renderJourney(root, map, name, journey, { resolve = true } = {}) {
  ...
  hops.forEach((hop, i) => {
    ...
    if (hop.outbound) {
      const contract =
        (Object.hasOwn(map.contracts ?? {}, hop.outbound) ? map.contracts[hop.outbound] : null) ??
        findNewContract(journey, hop.outbound)
...
export function acceptDraft(root, map, mapPath, name, { force = false, keepDraft = false } = {}) {
  ...
  for (const contract of newContracts) {
    ...
    // hasOwn, matching the check below: bracket access made an inherited id look defined, so
    // the contract was silently not added and then reported as missing by its own sibling.
    if (!Object.hasOwn(map.contracts ?? {}, contract.id)) {
  ...
  for (const hop of hops) {
    for (const id of [hop.inbound, hop.outbound]) {
      // hasOwn: an id like `constructor` is inherited and truthy, so finalize would never warn
      // that the contract is undefined.
      if (id && !Object.hasOwn(map.contracts ?? {}, id)) missingContracts.push(id)
    }
  }
