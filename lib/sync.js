...
export function syncRepo(root, id, repo, { mode = 'full', refresh = false, paths = [], widen = false, offline = false } = {}) {
  if (!repo?.url) {
  ...
  const cone = fresh ? null : reconcileSparse(dir, mode, paths, widen, offline)
...
function reconcileSparse(dir, mode, paths, widen, offline) {
  ...
  if (mode === 'full') {
    ...
    if (isSparse) {
      ...
      if (!widen) return { narrowed: true, reason: offline ? 'declined' : 'by-design' }
