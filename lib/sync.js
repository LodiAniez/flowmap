...
export function syncRepo(root, id, repo, { mode = 'full', refresh = false, paths = [], widen = false } = {}) {
  ...
  if (fresh) {
    ...
    // gains a hop or an anchor moves. Re-apply it — but only to a checkout that is already
    // sparse. Narrowing a full clone here would silently truncate the working tree that
    // `search` and `draft` grep, and nothing would ever widen it again.
  }
  ...
  return {
    ...
    narrowed: Boolean(cone?.narrowed),
...
function reconcileSparse(dir, mode, paths, widen) {
  ...
  if (mode === 'full') {
    ...
    if (isSparse && widen) {
      ...
      // origin is unreachable. Silently continuing would let search grep a coned tree and
      // present the result as a complete sweep — the exact failure widening exists to prevent.
      const ok = git(['sparse-checkout', 'disable'], { cwd: dir, allowFail: true })
      if (ok === null) return { narrowed: true }
    }
    return
  }
