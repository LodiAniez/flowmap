...
export function syncRepo(
  ...
  id,
  repo,
  { mode = 'full', refresh = false, paths = [], widen = false, offline = false, blockedByFlag = false } = {}
) {
  if (!repo?.url) {
  ...
  if (fresh) {
    ...
    if (mode === 'sparse' && paths.length) {
      writeFileSync(sparseMarker(dir), '')
    ...
    // Under --no-sync, leave the stale url in place. Correcting it without fetching records
    // the change as done, so no later run ever performs the fetch it implies — and search and
    // draft go on grepping the previous repository's files, reporting them as complete.
    if (configured && configured !== source && offline) {
      // Deferred, not done — and the tree is still the previous repository's, so the caller
      // has to be told rather than shown a result that looks complete.
      staleOrigin = true
    }
    if (configured && configured !== source && !offline) {
      // If re-pointing fails, fetching would pull from the *old* remote and the tree would
      // still hold the previous repository's files while we reported a successful re-point.
      const pointed = git(['remote', 'set-url', 'origin', source], { cwd: dir, allowFail: true })
      if (pointed === null) throw new UserError(`repo "${id}": could not re-point origin to ${source}`)
      // Re-point without re-fetching and the working tree still holds the *old* repository's
      // files, which search and draft would then grep as though they were the new one's. But
      // --no-sync promised no network, and honouring that beats correcting the tree.
      repointed = !offline
    }
  }

  if (!fresh && (refresh || repointed)) {
    const branch = repo.branch || currentBranch(dir)
    withBranchDiagnostics(id, { ...repo, branch }, source, () => {
  ...
  const cone = fresh ? null : reconcileSparse(dir, mode, paths, widen, blockedByFlag)
  ...
  return {
    ...
    // The checkout still belongs to a repository the registry no longer names.
    staleOrigin,
    narrowed: Boolean(cone?.narrowed) || freshNarrowed,
    narrowedReason: cone?.reason ?? (freshNarrowed ? 'unreachable' : null),
    refreshed: !fresh && (refresh || repointed),
...
// Marks a checkout flowmap created sparse. `core.sparseCheckout` cannot answer this: widening
// for `search` clears it, and reading it afterwards makes the checkout look like one the user
// cloned in full — so verify declines to re-narrow and every later run fetches the whole repo.
function sparseMarker(dir) {
  return join(dir, '.git', 'flowmap-sparse')
}

function reconcileSparse(dir, mode, paths, widen, blockedByFlag) {
  const isSparse = git(['config', 'core.sparseCheckout'], { cwd: dir, allowFail: true })?.trim() === 'true'
  // Ours to narrow if we made it sparse, whatever its current state.
  const ours = isSparse || existsSync(sparseMarker(dir))
  ...
  if (mode === 'full') {
    ...
    if (isSparse) {
      ...
      // Three reasons, three different fixes: the caller chose not to widen (nothing is
      // wrong), --no-sync forbade the fetch, or the fetch was attempted and failed.
      // `declined` only when the caller wanted to widen and the flag stopped it.
      if (!widen) return { narrowed: true, reason: blockedByFlag ? 'declined' : 'by-design' }
      const ok = git(['sparse-checkout', 'disable'], { cwd: dir, allowFail: true })
      if (ok === null) return { narrowed: true, reason: 'unreachable' }
    }
    return
  }
  // Narrow only a checkout flowmap made sparse. Narrowing one the user cloned in full would
  // truncate the tree other commands depend on.
  if (mode === 'sparse' && ours && paths.length) {
    const ok = git(['sparse-checkout', 'set', ...paths], { cwd: dir, allowFail: true })
    if (ok === null) {
