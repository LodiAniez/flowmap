...
export function verify(root, map, mapPath, { repoIds = null, journeys = null, now = new Date() } = {}) {
  ...
  for (const id of targets) {
    ...
    try {
      ...
      // One round per import level, syncing between them: level 2 can only be discovered by
      // reading level-1 files, and those are not on disk until the level-1 widening lands.
      let cone = base
  ...
  const contracts = contractIds.length
    ? checkContracts(root, map, { contractIds, synced, scoped: scoped || unreachable.length > 0 })
    : []
