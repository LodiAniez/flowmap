...
export function verify(root, map, mapPath, { repoIds = null, journeys = null, now = new Date() } = {}) {
  ...
  // Only work actually done: a contract that was enumerated and skipped (no schema, not a
  // path, repo not synced) proves nothing was verified.
  const checked = rows.length + contracts.filter((c) => c.status === SCHEMA_OK || c.missing.length).length
