...
export function verify(root, map, mapPath, { repoIds = null, journeys = null, now = new Date() } = {}) {
  ...
  const failedRepos = new Set(repos.filter((r) => r.error).map((r) => r.id))
  ...
  const stranded = [...strandedByFailure].filter((id) => !inScope.has(id))
  const outOfScope = [...strandedByScope].filter((id) => !inScope.has(id) && !strandedByFailure.has(id))
