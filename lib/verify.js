...
function relativeSpecsOf(root, repoId, filePath) {
  ...
  const patterns = [
    ...
    // JSON Schema / OpenAPI. lib/contracts.js follows these, so the cone must fetch them or
    // the checker reads a file that was never there and downgrades a real verdict.
    /"\$ref"\s*:\s*"(\.[^"#]*)"/g,
  ]
  let m
...
// Every contract id some hop carries. Both callers below walked every journey to build this
// independently, which is one traversal of the whole map each.
function carriedContracts(map) {
...
function unusedReposReason(map, contracts) {
  ...
  const carried = carriedContracts(map)
...
// Whether the repo a contract's schema names was among those this run searched. Only a
// repo-qualified ref names one; a bare path could be anywhere, so the answer is no.
function schemaRepoSearched(map, contractId, synced) {
  const contract = Object.hasOwn(map.contracts ?? {}, contractId) ? map.contracts[contractId] : null
  if (!contract) return false
  const ref = parseSchemaRef(contract?.schema, Object.keys(map.repos ?? {}))
  return ref.kind === 'path' && Boolean(ref.repo) && synced.has(ref.repo)
}
...
export function verify(root, map, mapPath, { repoIds = null, journeys = null, self = null, now = new Date() } = {}) {
  ...
  for (const id of targets) {
    ...
    // schemaOnly first: its `entries` were forced to [], and when the run is scoped by repoIds
    // alone `byRepo` and `everyAnchor` are the same object, so covers() compares 0 against 0
    // and passes. The `all === 0` guard inside covers() only catches a repo with no anchors
    // anywhere — not the common shared-contracts repo that also hosts hops.
    const complete = !schemaOnly.has(id) && covers(id) && !synced.narrowed
    repos.push({
      recorded: complete,
  ...
  for (const name of journeys ?? Object.keys(map.journeys ?? {})) {
    for (const hop of journey(map, name)?.hops ?? []) {
      for (const id of [hop.inbound, hop.outbound]) {
        if (!id) continue
        // Checkable when the schema's own repo was searched, whatever happened to the hop's.
        // A hop with no anchors — a queue or broker step — never becomes a sync target, and
        // gating on it alone skipped contracts whose schema was sitting right there.
        if (synced.has(hop.repo) || schemaRepoSearched(map, id, synced)) inScope.add(id)
  ...
  // Only contracts a run could have checked. One the map does not define is a map defect
  // acceptDraft already reports; one with no schema at all has nothing to check, so listing it
  // as skipped tells the reader — and the agent — about work that never existed.
  const defined = (id) => {
    if (!Object.hasOwn(map.contracts ?? {}, id)) return false
    const ref = parseSchemaRef(map.contracts[id]?.schema, Object.keys(map.repos ?? {}))
    return ref.kind === 'path'
  }
  // One bucket per contract, in order of how actionable the cause is: a sync failure and a
  // registry defect each need a specific fix, an incomplete checkout needs a retry, and being
  // outside the scope needs nothing. Overlapping them produced two TSV rows for one contract
  // with contradictory statuses, and double-counted the totals.
  const claimed = new Set()
  const take = (source) =>
    [...source].filter((id) => {
      if (!defined(id) || inScope.has(id) || claimed.has(id)) return false
      claimed.add(id)
      return true
    })

  const stranded = take(strandedByFailure)
  const unregistered = take(strandedByRegistry)
  const partial = take(strandedByPartial)
  const outOfScope = take(strandedByScope)
