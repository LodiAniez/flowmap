...
function pathsFor(entries, map, repoIds) {
  ...
  // Schemas compose across sibling directories — `src/schemas/order.ts` importing
  // `../types/base.ts` is the ordinary layout. A cone directory is recursive but has no
  // siblings, so narrowing to the schema's own directory leaves its imports unreadable and
  // downgrades every real verdict to "could not tell". Take the top-level directory instead:
  // far short of the whole repo, but wide enough to hold what a schema pulls in.
  for (const contract of Object.values(map.contracts ?? {})) {
    const ref = parseSchemaRef(contract?.schema, repoIds)
    if (ref.kind !== 'path') continue
    if (ref.path.includes('/')) paths.add(ref.path.split('/')[0])
    else add(ref.path)
  }
  return [...paths]
}
  ...
  for (const id of targets) {
    ...
    try {
      synced = syncRepo(root, id, config, {
        ...
        paths: pathsFor(everyAnchor.get(id) ?? [], map, Object.keys(map.repos ?? {})),
      })
    } catch (err) {
