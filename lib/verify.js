...
function allBareResolved(map, contracts) {
  ...
  for (const [id, contract] of Object.entries(map.contracts ?? {})) {
    ...
    if (!byId.get(id)?.repo) return false
  }
  return true
}
...
export function unusedRepos(map, { foundIn = [], bareResolved = null, drafts = [] } = {}) {
  const ids = Object.keys(map.repos ?? {})
  ...
  for (const name of journeys ?? Object.keys(map.journeys ?? {})) {
    for (const hop of journey(map, name)?.hops ?? []) {
      for (const id of [hop.inbound, hop.outbound]) {
        if (!id) continue
        // A contract carried only by hops in a repo that failed to sync is not checkable —
        // but dropping it silently is the map-wide vanishing this is supposed to prevent.
        if (synced.has(hop.repo)) inScope.add(id)
        else strandedByRepo.add(id)
      }
    }
  }
  const stranded = [...strandedByRepo].filter((id) => !inScope.has(id))
