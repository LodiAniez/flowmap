...
// Directories a repo's schemas import from that the current cone does not already cover.
// Two rounds, matching the import depth the contract check follows.
function importDirs(root, repoId, map, repoIds, have) {
  ...
  for (const contract of Object.values(map.contracts ?? {})) {
    ...
    if (ref.kind === 'path') frontier.push(ref.path)
  }
...
function relativeImportsOf(root, repoId, filePath) {
  ...
  while ((m = re.exec(source))) {
    ...
    const base = posix.normalize(posix.join(posix.dirname(filePath), m[1])).replace(/\.(js|mjs|cjs)$/, '')
    if (base.startsWith('..')) continue
    for (const ext of ['.ts', '.tsx', '.js', '/index.ts', '/index.js']) out.push(base + ext)
  }
  return out
}
...
export function verify(root, map, mapPath, { repoIds = null, journeys = null, now = new Date() } = {}) {
  ...
  // look everywhere. A scoped run syncs just the repos a schema ref names outright.
  const alsoSync = scoped ? schemaRepos(map) : Object.keys(map.repos ?? {})
  ...
  for (const id of targets) {
    ...
    try {
      ...
      const base = pathsFor(everyAnchor.get(id) ?? [], map, Object.keys(map.repos ?? {}))
      ...
      const extra = importDirs(root, id, map, Object.keys(map.repos ?? {}), base)
      if (extra.length) {
  ...
  // A repo that errored is not evidence of absence — it is evidence we could not look.
  const unreachable = repos.some((r) => r.error)
  const contracts = contractIds.length
    ? checkContracts(root, map, { contractIds, synced, scoped: scoped || unreachable })
    : []
