...
function importDirsOnce(root, repoId, map, repoIds, have) {
  ...
  for (const filePath of schemaFilesOnDisk(root, repoId, map, repoIds)) {
    for (const dep of relativeImportsOf(root, repoId, filePath)) {
      ...
      // `./base.ts` resolves directly, so the `/index.ts` candidate built from it yields
      // `src/schemas/base.ts` as a "directory". Cone mode rejects a file path and fails the
      // whole command, and the fallback disables sparse for good — nothing re-narrows a
      // checkout that is no longer sparse.
      if (key !== '.' && isFileOnDisk(root, repoId, key)) continue
      found.add(key)
    }
  }
  return [...found]
...
function isFileOnDisk(root, repoId, rel) {
  const full = join(cacheDir(root), repoId, rel)
...
export function unusedRepos(map, { foundIn = [] } = {}) {
  ...
  // A bare path does not say which repo holds it. Where the run actually located those
  // schemas, `foundIn` covers it; where it did not, we cannot claim a repo is unused —
  // telling someone to remove the repo their schemas live in would break their map.
  if (bare && !foundIn.length) return []
...
export function verify(root, map, mapPath, { repoIds = null, journeys = null, now = new Date() } = {}) {
  ...
  const checked = rows.length + contracts.length
  ...
  return {
    ...
    unusedRepos: unusedRepos(map, { foundIn: contracts.map((c) => c.repo).filter(Boolean) }),
