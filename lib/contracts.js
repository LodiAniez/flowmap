...
function importSpecs(source) {
  ...
  const bare = [
    /\bimport\s+['"]([^'"]+)['"]/g,
    /\brequire\(\s*['"]([^'"]+)['"]\s*\)/g,
    // Dynamic `import('./x')`. lib/verify.js already widens the cone for these, so without
    // this the file is fetched and then ignored — and a field living in it is reported
    // definitely missing rather than unknown.
    /\bimport\s*\(\s*['"]([^'"]+)['"]\s*\)/g,
  ]
  ...
  while ((m = withBindings.exec(source))) {
    ...
    let names
    if (/^\s*\*/.test(clause) && !alias.length) {
      // `export * from 'pkg'` republishes whatever the package declares — the shape is hidden
      // wholesale and there is no binding to look for.
      names = ['*']
    } else if (/^\s*\*/.test(clause)) {
      // `import * as z from 'zod'` only binds a namespace object. Whether it contributes to
      // the shape is decided by how that object is used, exactly like a named import — the
      // blanket treatment made every `import * as z from 'zod'` hide a real missing field.
      names = alias
    } else {
      names = [...clause.matchAll(/[A-Za-z_$][\w$]*/g)].map((x) => x[0]).filter((n) => n !== 'type')
    }
...
function composesFrom(source, binding) {
  ...
  return new RegExp(
    ...
    `\\b${b}\\s*\\.(\\s*\\w+\\s*\\.)*\\s*(extend|merge|omit|pick|partial|required|and|or|shape|catchall)\\b` +
      `|\\.\\.\\.\\s*${b}\\b` +
      // combinator-style: the binding is an argument, not the receiver
      `|\\b(intersection|union|discriminatedUnion|and|or|merge|allOf|oneOf|anyOf)\\s*\\([^)]*\\b${b}\\b` +
