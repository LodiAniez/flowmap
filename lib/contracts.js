...
import { join, dirname, posix, normalize } from 'node:path'
...
export function checkContract(root, map, id, contract, { synced = null } = {}) {
  ...
  const searchable = candidateRepos(map, id, ref.repo).filter((r) => !synced || synced.has(r))
  let source = null
  let foundIn = null
  ...
  if (source === null) {
    ...
    // Only claim "nowhere" when every repo that could plausibly own the schema was searched.
    // Measuring against the full registry made a genuine miss unreportable; measuring only
    // "did we search anything" reported healthy contracts as missing under --local.
    const candidates = candidateRepos(map, id, ref.repo)
    const unsearched = synced && searchable.length < candidates.length
    return {
      id,
