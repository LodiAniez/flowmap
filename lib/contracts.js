  ...
  for (let depth = 0; depth < IMPORT_DEPTH; depth++) {
    ...
    for (const file of frontier) {
      for (const { spec, bindings } of importSpecs(file.body)) {
        if (!spec.startsWith('.')) {
          const opaque = bindings.length
            ? bindings.some((b) => composesFrom(file.body, b))
            : includesByWholeFile(file.path)
          if (opaque) external = true
          continue
        }
  ...
  for (const file of lastRoundFiles) {
    for (const onward of importSpecs(file.body)) {
      if (!onward.spec.startsWith('.')) {
        // Same rule as the main walk, at whatever depth we met it.
        const opaque = onward.bindings.length
          ? onward.bindings.some((b) => composesFrom(file.body, b))
          : includesByWholeFile(file.path)
        if (opaque) external = true
        continue
      }
...
function importSpecs(source) {
  ...
  // JSON Schema and OpenAPI compose by reference rather than by import, and carry no bindings.
  // The fragment after `#` addresses a location inside the target, not a different file.
  const ref = /"\$ref"\s*:\s*"([^"#][^"]*)"/g
  while ((m = ref.exec(source))) out.push({ spec: m[1].split('#')[0], bindings: [] })
...
// Line and block comments removed, so a commented-out import is not read as a real one. Kept
// deliberately simple: this only has to stop dead specs being followed, and a quote-aware
// tokeniser would be a parser, which this file exists to avoid.
...
// Whether a bindingless specifier in this file means the shape continues elsewhere.
//
// In protobuf and JSON Schema an include *is* the composition — there are no bindings to
// judge. In JS/TS a bindingless import is a side effect (`import 'reflect-metadata'`,
// `import './polyfills'`, a bare `require('dotenv/config')`) and contributes no field, so
// treating it as opaque let one such line anywhere in the import graph turn every real
// verdict into "could not tell".
function includesByWholeFile(path) {
  return /\.(proto|json|ya?ml|avsc|avro|xsd|graphqls?|gql)$/i.test(path ?? '')
}
