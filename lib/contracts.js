  ...
  for (let depth = 0; depth < IMPORT_DEPTH; depth++) {
    ...
    for (const file of frontier) {
      for (const { spec, bindings } of importSpecs(file.body)) {
        if (!spec.startsWith('.')) {
          // A bindingless non-relative spec is a whole-file include — `import "x.proto";` in
          // protobuf, or a JSON Schema `$ref` — so the shape genuinely continues somewhere we
          // are not reading. Only a JS/TS import binds names we can judge individually.
          if (!bindings.length || bindings.some((b) => composesFrom(file.body, b))) external = true
          continue
        }
  ...
  for (const file of lastRoundFiles) {
    for (const onward of importSpecs(file.body)) {
      if (!onward.spec.startsWith('.')) {
        // Same rule as the main walk: a bindingless non-relative spec is a whole-file include,
        // so the shape continues somewhere unreadable whatever depth we met it at.
        if (!onward.bindings.length || onward.bindings.some((b) => composesFrom(file.body, b))) {
          external = true
        }
        continue
      }
