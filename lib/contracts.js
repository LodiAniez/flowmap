...
function importSpecs(source) {
  ...
  while ((m = withBindings.exec(source))) {
    const clause = m[1]
    // `export * from 'pkg'` and `import * as X from 'pkg'` both pull in an unknown shape.
    const names = /\*/.test(clause)
      ? ['*', ...[...clause.matchAll(/\bas\s+([A-Za-z_$][\w$]*)/g)].map((x) => x[1])]
      : [...clause.matchAll(/[A-Za-z_$][\w$]*/g)].map((x) => x[0]).filter((n) => n !== 'type')
