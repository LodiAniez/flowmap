...
function looksLikePath(ref) {
  ...
  return /\.[a-z0-9]{1,10}$/.test(ref)
}

export function parseSchemaRef(ref, repoIds) {
  if (!ref) return { kind: NO_SCHEMA }
  if (!looksLikePath(ref)) return { kind: NOT_A_PATH, ref }
...
export function checkContract(root, map, id, contract, { synced = null } = {}) {
  ...
  if (source === null) {
    ...
    // Compare against what we could actually look in. Measuring against the full registry
    // made a genuine "not found anywhere" unreachable as soon as one hopless repo existed.
    const unsearched = synced && searchable.length === 0
    return {
      id,
