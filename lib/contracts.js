...
function gatherSources(root, repoId, entryPath, entryBody) {
  ...
  // Depth exhausted with files still unexplored is the same kind of unknown as an
  // unreadable import: part of the shape may live one hop further out than we looked.
  if (frontier.length) external = true
...
// Which repos might hold this contract's schema. Contracts do not record their owning repo,
// so this is partly inference — and the distinction matters:
//
//   known   the ref names a repo, or a hop carries this contract. A repo we did not search
//           genuinely leaves the question open.
//   guessed nothing points anywhere, so we sweep the registry. A guess must never justify
//           "we did not look properly" — that made the real "not found" verdict unreachable
//           as soon as one hopless repo was registered, which is the normal state.
function candidateRepos(map, contractId, hinted) {
  if (hinted) return { repos: [hinted], guessed: false }
  ...
  if (touching.size) {
    ...
    return { repos: [...touching, ...rest], known: touching.size, guessed: false }
  }
  return { repos: Object.keys(map.repos ?? {}), guessed: true }
}
...
export function checkContract(root, map, id, contract, { synced = null } = {}) {
  ...
  // Only the repos we had reason to believe in count towards "did we look properly".
  const expected = candidates.guessed ? 0 : (candidates.known ?? candidates.repos.length)
  const searchedKnown = candidates.repos.slice(0, expected).filter((r) => !synced || synced.has(r)).length
  let source = null
  let foundIn = null
  ...
  if (source === null) {
    ...
    // "Nowhere" is only honest once every repo we had reason to check was checked. A guessed
    // sweep cannot leave the question open — but searching nothing at all still does.
    const unsearched = synced && (candidates.guessed ? searchable.length === 0 : searchedKnown < expected)
    return {
      id,
...
export function checkContracts(root, map, { contractIds = null, synced = null } = {}) {
  const entries = Object.entries(map.contracts ?? {}).filter(([id]) =>
    contractIds ? contractIds.includes(id) : true
  )
  return entries.map(([id, contract]) => checkContract(root, map, id, contract, { synced }))
}
