    ...
    for (const file of frontier) {
      for (const { spec, bindings } of importSpecs(file.body)) {
        ...
        if (lastRound) {
          ...
          for (const onward of importSpecs(found.body)) {
            if (!onward.spec.startsWith('.')) {
              // A package composed into the shape hides it just as much at this level.
              if (onward.bindings.some((b) => composesFrom(found.body, b))) external = true
              continue
            }
            // Only an import leading somewhere we have not already read is a genuine unknown.
            const onwardPath = resolveImport(root, repoId, found.path, onward.spec)
...
function importSpecs(source) {
  ...
  // Forms with no `from`: side-effect imports, require(), and dynamic import().
  const bare = [
    /\bimport\s+['"]([^'"]+)['"]/g,
    /\brequire\(\s*['"]([^'"]+)['"]\s*\)/g,
    // lib/verify.js already widens the cone for dynamic imports, so without this the file is
    // fetched and then ignored, and a field living in it is reported definitely missing.
    /\bimport\s*\(\s*['"]([^'"]+)['"]\s*\)/g,
  ]
