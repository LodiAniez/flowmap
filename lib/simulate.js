import { normalizeField } from './fields.js'
...
export function simulate(map, journey, input) {
  ...
  for (const [i, hop] of hops.entries()) {
    ...
    if (hop.inbound && !emitted.has(hop.inbound)) {
      ...
      if (entry) {
        payload = Object.fromEntries(
          (entry.fields ?? []).map(normalizeField).map((f) => [f.name, sampleFor(f)])
        )
    ...
    const declared = contract ? (contract.fields ?? []).map((f) => normalizeField(f).name) : null
