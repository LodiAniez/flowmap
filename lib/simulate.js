...
// hasOwn, not bracket access: `contracts.constructor` is inherited and truthy, so an
// undefined contract id would look defined — `declared` becomes [] rather than null and every
// payload key is then reported as not in the contract.
function contractOf(map, id) {
  return Object.hasOwn(map.contracts ?? {}, id) ? map.contracts[id] : null
}
...
export function simulate(map, journey, input) {
  ...
  for (const [i, hop] of hops.entries()) {
    ...
    if (hop.inbound && !emitted.has(hop.inbound)) {
      const entry = contractOf(map, hop.inbound)
      if (entry) {
    ...
    const contract = hop.outbound ? contractOf(map, hop.outbound) : null
