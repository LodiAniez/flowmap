...
export const BOOLEAN_FLAGS = new Set([
  ...
  'keep-draft', 'keep-path', 'mermaid', 'brief', 'discover', 'agent', 'help', 'h', 'i',
])

// `--check` is deliberately absent above: it is dual-form. `--check <name>` and `--check=<name>`
// both name a draft, and bare `--check` falls back to the trailing positional.
...
function set(flags, key, value) {
  if (BOOLEAN_FLAGS.has(key)) {
    ...
    // Anything explicitly falsy is honoured; anything else means "on". An empty value is
    // neither — almost always an unset shell variable — and is left for the caller to reject,
    // matching how `--repos=` is treated.
  ...
  // hasOwn, not `in`: flags is a plain object, so `'constructor' in flags` is true on the
  // first occurrence and the value becomes [Object, 'bar']. Three other lookups were converted
  // for this reason; the parser itself was missed.
